...
ON (DEPT_CODE = DEPT_ID); -- 23행 EMPLOYEE 기준 (하동운, 이오리 포함)
...
-- 2) RIGHT [OUTER] JOIN : 합치기에 사용된 두 테이블 중 오른편에 기술된 테이블의
-- 컬럼수를 기준으로 JOIN

-- ANSI 방식
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE  RIGHT JOIN DEPARTMENT
ON (DEPT_CODE = DEPT_ID); -- 24행 DEPARTMENT 기준 (마케팅부, 국내영업부, 해외영업3부 포함)
...
JOIN LOCATION ON (LOCATION_ID = LOCAL_ID)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
-- SQL Error [904] [42000]: ORA-00904: "LOCAL_ID": 부적합한 식별자
--> EMPLOYEE 테이블에 LOCATION_ID 컬럼이 없어서 오류발생
--> 해결 방법 : DEPARTMENT와 LOCATION 조인 순서를 바꿔서
-- 				EMPOLYEE와 DEPARTMENT가 조인된 결과를 먼저 만들어
...
-- EMPLOYEE / JOB / DEPARTMENT / LOCATION/ EMPLOYEE
...
-- WHERE DEPT_TITLE LIKE '%1부' OR DEPT_TITLE '%2부';
...
SELECT EMP_NAME, JOB_NAME, SALARY 급여, SALARY * (1 + NVL(BONUS,0) ) * 12 연봉
FROM EMPLOYEE
NATURAL JOIN JOB
