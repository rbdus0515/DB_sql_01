...
SELECT INSTR('AABAACAABBAA' , 'B' , 5, 2) FROM DUAL; -- 9
...
WHERE ABS(10) = ABS(-10); -- WHERE절 함수 작성
...
SELECT TRUNC(123.456, 1) FROM DUAL; -- 123.4, 소수점 첫째자리 아래 버림
...
-- EMPLOYEE 테이블에서 보너스를 받으면'O', 안바등면 'X' 조회
...
SELECT EMP_NAME, JOB_CODE, SALARY,
	DECODE(JOB_CODE, 'J7' , '20%',
					  ...
					  'J5' , '10%',
					  '5%') 인상률,
	DECODE(JOB_CODE , 'J7', SALARY +(SALARY * 1.2),
						'J6', SALARY +(SALARY * 1.15),
						'J5', SALARY +(SALARY * 1.1),
						SALARY +(SALARY * 1.05)) 인상된 급여
FROM EMPLOYEE;
...
----- 그룹 함수 -----
-- 하나 이상의 행을 그룹으로 묶어 연산하여 총합, 평균등의 하나의 결과 행으로 반환하는 함수
...
-- 급여 최대값, 가장 늦은 입사일, 알파벳 순서가 가장 느른 이메일
